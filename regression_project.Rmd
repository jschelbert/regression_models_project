...
It can be shown that an automatic transmission yields a significant higher average miles per gallon value if we only use transmission as a confounding variable.
However, if we also account for other confouding variables the significance for the influence of the transmission type vanishes.
...
dtcars$am <- factor(dtcars$am, labels=c("manual", "automatic"))
...
Based on that we reject the null hypothesis and conclude that the group with the automatic transmission has higher average miles per galon.
More precisely, a manual transmission has an average mpg of `r t_test$estimate[[1]]` while an automatic yields `r t_test$estimate[[2]]`.
...
We observe an intercept of `r sumCoef[1,1]` which corresponds to the mean mpg for a manual transmission.
The coefficient for automatic transmission `r sumCoef[2,1]` correspoinds to the additonal miles per gallon, meaning an automatic transmission will result a `r sumCoef[1,1] + sumCoef[2,1]` mpg on average.
One might notice that these numbers are the same as the ones found by the t-test in the previous section.
The 95% confidence interval for the automatic transmission coefficient is $[`r conf_int_fit1`]$, thus we can be quite confident that an automatic transmission has a positive impact on the miles per gallon within that range.
...
Obviously the weight of a car negatively affects the miles per gallon, while an automatic transmission and the 1/4 mile time have a positive influence on mpg.
Compared to the model that just uses transmission type the coeffienct of the transmission is now smaller.
We conclude that for this model that with 95% confidence an automatic transmission increases the miles per gallon in the interval $[`r conf_int_bestfit`]$.
...
It can be shown that an automatic transmission has significant positive influence on the mpg value.
...
In this model the influence of the transmission type is `r sumCoef_bf[4,1]` increase in miles per gallon for an automatic transmission.
